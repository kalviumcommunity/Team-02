...
export const CreateRequestModal = ({ onClose }) => {
  const { products, activeStoreId, stores, createRequest } = useApp();
  ...
  const [lines, setLines] = useState([
    { productId: products[0]?.id || 'SKU-8821', requestedQty: 25 }
  ]);
  ...
  const handleSubmit = (asDraft = false) => {
    if (priority === 'Urgent' && !urgencyReason && !asDraft) {
      alert('Please enter an urgency justification reason for urgent requests.');
      return;
    }
